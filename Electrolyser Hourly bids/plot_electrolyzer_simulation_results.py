# plot_electrolyzer_simulation_results.py
# V16 - Applied consistent plotting enhancements from steel plant script.
# - Storage and Electricity plots now have market price subplots.
# - Storage plot y-axis is now dynamic.
# - Forecast plot is cleaned of metric text.
# - Plant operations plot was already based on electricity, no change needed.

"""
Generates plots for visualizing electrolyzer simulation results including:
- A simplified plant analysis focusing on electricity, market price, and deviation.
- Greatly improved price scenario "fan charts" for clear percentile visualization.
- Redesigned cumulative production plot with separate, properly scaled subplots.
- Storage levels for materials with market context and dynamic scaling.
- Price forecasts vs. actuals (directly from CSVs).
- Bidding curves.

Usage (VS Code / Terminal):
    1. Place this script in the same directory as your 'config.yaml' file.
...
def load_electrolyzer_data(output_dir, config):
    output_dir_basename = os.path.basename(output_dir.rstrip('/\\'))
    if not output_dir_basename:
        output_dir_basename = os.path.basename(os.path.abspath(output_dir))

    print(f"DEBUG: Using output file prefix derived from command line: '{output_dir_basename}'")
    ...
    files_to_try = {
        ...
        "hydrogen_prod_file_expected": f"{output_dir_basename}_hydrogen_produced_periodical_expected_main_opt.xlsx",
        ...
        "hydrogen_prod_file_actual": f"{output_dir_basename}_hydrogen_produced_periodical_actual.xlsx",
        ...
        "forecast_errors": f"{output_dir_basename}_forecast_errors.xlsx",
        "economic_indicators": f"{output_dir_basename}_economic_indicators.xlsx"
    }
    ...
    if not accepted_bids_df_full.empty and 'Date' in accepted_bids_df_full.columns:
    ...
    if market_input_file_from_config and os.path.exists(market_input_file_from_config):
        try:
            ...
            if not os.path.isabs(market_input_file_abs):
                # Assume config file is in the same dir as the script for relative path calculation
    ...
    return loaded_data, mcp_data_actual_unique, market_data_full
...
def plot_forecast_vs_actual_window(period_info, config, market_data_full, main_output_dir, plot_output_dir):
    ...
    MODIFIED to remove the error metrics text box.
    """
    ...
    try:
        ...
        # --- MODIFICATION: Removed the metrics text box ---
...
def plot_plant_data(plant_name, plant_df_expected, plant_df_actual, market_data_full, opportunity_cost, plot_output_dir):
    """
    Plots operational analysis for a single plant based on electricity consumption.
    This function was already aligned with the desired electricity-based plotting.
    """
    ...
    if not plant_df_expected.empty:
        # Ensure the correct column name is used if it differs
        expected_col = 'Expected_Absolute_Electricity_MWh' if 'Expected_Absolute_Electricity_MWh' in plant_df_expected.columns else 'Expected_Operation_Level'
        plot_df = plant_df_expected[['Timestamp', expected_col]].copy().rename(columns={expected_col: 'Expected_Absolute_Electricity_MWh'})

    if not plant_df_actual.empty:
        # Ensure the correct column name is used
        actual_col = 'Actual_Absolute_Electricity_MWh' if 'Actual_Absolute_Electricity_MWh' in plant_df_actual.columns else 'Actual_Operation_Level'
        actual_df_temp = plant_df_actual[['Timestamp', actual_col]].copy().rename(columns={actual_col: 'Actual_Absolute_Electricity_MWh'})
    ...
    plot_df['Elec_Deviation'] = plot_df['Actual_Absolute_Electricity_MWh'] - plot_df['Expected_Absolute_Electricity_MWh']

    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(18, 16), sharex=True,
                                        gridspec_kw={'height_ratios': [2, 1, 1.5]})
    fig.suptitle(f"Electrical Operational Analysis for {plant_name}", fontsize=18, y=0.98)
    
    ax1.set_title("Electricity Consumption (MWh)", fontsize=12)
    ax1.fill_between(plot_df['Timestamp'], 0, plot_df['Expected_Absolute_Electricity_MWh'], 
                     color='dodgerblue', alpha=0.3, label='Expected Electricity (MWh)')
    ax1.plot(plot_df['Timestamp'], plot_df['Actual_Absolute_Electricity_MWh'],
             label='Actual Electricity (MWh)', color='navy', linewidth=1.5)
    ax1.set_ylabel('Electricity (MWh)'); ax1.legend(loc='upper left')
    ax1.grid(True, linestyle=':', alpha=0.6)
    
    ax2.set_title("Electricity Deviation from Plan (Actual - Expected)", fontsize=12)
    ax2.plot(plot_df['Timestamp'], plot_df['Elec_Deviation'], 
              label='Electricity Deviation (MWh)', color='firebrick')
    ax2.axhline(0, color='black', linestyle=':', linewidth=1)
    ax2.set_ylabel('Deviation (MWh)')
...
def plot_storage_data(material_name, material_df_expected, material_df_actual, market_data_full, opportunity_cost, plot_output_dir):
    """
    Plots the hourly storage level for a single material with dynamic y-axis scaling
    and a subplot for the market clearing price.
    """
    ...
    fig.suptitle(f"Storage and Market Analysis for {material_name}", fontsize=16, y=0.98)
    ...
    if not material_df_expected.empty:
        ax1.plot(material_df_expected['Timestamp'], material_df_expected['Expected_Storage_Level_Tons'],
                 label=f'Expected {material_name} Level (tons)', color='teal', linewidth=1.5, alpha=0.8)
        max_data_level = max(max_data_level, material_df_expected['Expected_Storage_Level_Tons'].max())
        has_data = True

    if not material_df_actual.empty:
        ax1.plot(material_df_actual['Timestamp'], material_df_actual['Actual_Storage_Level_Tons'],
                 label=f'Actual {material_name} Level (tons)', color='brown', linestyle='--', linewidth=1.5, alpha=0.8)
        max_data_level = max(max_data_level, material_df_actual['Actual_Storage_Level_Tons'].max())
        has_data = True
    ...
    if pd.notna(min_level):
        ax1.axhline(y=min_level, color='red', linestyle=':', label=f'Min Level ({min_level:,.0f} tons)', linewidth=1)
    if pd.notna(max_level) and max_level < 1e8: # Avoid plotting huge numbers if max is infinite
        ax1.axhline(y=max_level, color='green', linestyle=':', label=f'Max Level ({max_level:,.0f} tons)', linewidth=1)

    if has_data and max_data_level > 0:
    ...
    ax1.set_ylabel(f'{material_name} Level (tons)')
    ...
    if not market_data_full.empty:
        min_ts = material_df_expected['Timestamp'].min() if not material_df_expected.empty else (material_df_actual['Timestamp'].min() if not material_df_actual.empty else None)
        max_ts = material_df_expected['Timestamp'].max() if not material_df_expected.empty else (material_df_actual['Timestamp'].max() if not material_df_actual.empty else None)
...
def plot_cumulative_hydrogen_production(h2_df_expected, h2_df_actual, market_data_full, opportunity_cost, config, plot_output_dir):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(18, 12), sharex=True, 
                                   gridspec_kw={'height_ratios': [2, 1]})
    fig.suptitle('Hydrogen Production and Market Price Analysis', fontsize=16, y=0.99)

    ax1.set_title('Cumulative Hydrogen Production', fontsize=12)
    plot_something = False

    if not h2_df_expected.empty:
        df_exp = h2_df_expected.sort_values(by='end_date').copy()
        df_exp['cumulative'] = df_exp['total_expected_hydrogen_kg'].cumsum()
        ax1.plot(df_exp['end_date'], df_exp['cumulative'], label='Cumulative Expected Hydrogen (kg)', color='blue', marker='o', linestyle='-', markersize=4, linewidth=2, alpha=0.8)
        plot_something = True

    if not h2_df_actual.empty:
        df_act = h2_df_actual.sort_values(by='end_date').copy()
        df_act['cumulative'] = df_act['total_actual_hydrogen_kg'].cumsum()
        ax1.plot(df_act['end_date'], df_act['cumulative'], label='Cumulative Actual Hydrogen (kg)', color='green', marker='x', linestyle='--', markersize=5, linewidth=2, alpha=0.8)
        plot_something = True
    ...
    if target_h2_per_day > 0 and sim_start_date_str and sim_end_date_str:
    ...
    ax2.set_title('Market Clearing Price & Opportunity Cost', fontsize=12)
    if not market_data_full.empty:
        min_ts = h2_df_expected['start_date'].min() if not h2_df_expected.empty else (h2_df_actual['start_date'].min() if not h2_df_actual.empty else pd.NaT)
        max_ts = h2_df_expected['end_date'].max() if not h2_df_expected.empty else (h2_df_actual['end_date'].max() if not h2_df_actual.empty else pd.NaT)
        
        if pd.notna(min_ts) and pd.notna(max_ts):
            mcp_plot_df = market_data_full[(market_data_full['Timestamp'] >= min_ts) & (market_data_full['Timestamp'] <= max_ts)]
            ax2.plot(mcp_plot_df['Timestamp'], mcp_plot_df['MCP'], color='purple', linewidth=1.5, label='Actual MCP (€/MWh)')
        else:
             ax2.text(0.5, 0.5, 'Date range for MCP not available.', ha='center', va='center', transform=ax2.transAxes)
    else:
        ax2.text(0.5, 0.5, 'Complete market data not available.', ha='center', va='center', transform=ax2.transAxes)
    ...
    plot_filename = os.path.join(plot_output_dir, "cumulative_hydrogen_production_analysis.png")
    plt.savefig(plot_filename, dpi=300)
    plt.close(fig)

def plot_total_electricity_overview(consumption_data_exp, accepted_bids_df, consumption_data_act, market_data_full, opportunity_cost, config, plot_output_dir):
    """Plots an overview of total hourly electricity consumption components and market price."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(18, 12), sharex=True,
                                   gridspec_kw={'height_ratios': [2, 1]})
    fig.suptitle('Total Electricity and Market Price Overview', fontsize=16, y=0.98)
    ...
    if 'Accepted MW' in plot_df.columns:
        ax1.plot(plot_df['Timestamp'], plot_df['Accepted MW'], label='Total Cleared from Market', color='black', marker='.', markersize=3, alpha=0.9, linewidth=1.5)
    ...
    if 'Actual_Absolute_Electricity_MWh' in plot_df.columns:
        ...
        ax1.plot(plot_df['Timestamp'], plot_df['Total_Actual_Consumed'], label='Total Actual Consumption', color='green', linestyle='-', alpha=0.9, linewidth=1.5)
    ...
    if 'Expected_Absolute_Electricity_MWh' in plot_df.columns:
        ...
        ax1.plot(plot_df['Timestamp'], plot_df['Total_Expected_Consumed'], label='Total Expected Consumption', color='dodgerblue', linestyle='--', alpha=0.8, linewidth=1.5)
...
def plot_bidding_curves_for_day(day_bids_df, plot_date, plot_output_dir):
    ...
    hours_to_plot = [h for h in hours_to_plot if h in hours_in_data]
...
def main(output_dir_arg, config_file_arg):
    print(f"Starting visualization process for electrolyzer model outputs in: {output_dir_arg}")
    config = load_config(config_file_arg)
    if config is None: sys.exit(1)

    loaded_data, mcp_data_actual_unique, market_data_full = load_electrolyzer_data(output_dir_arg, config)
    plot_main_dir = os.path.join(output_dir_arg, "simulation_plots_electrolyzer")
    ...
    all_plants = set()
    if not consumption_data_exp.empty:
        all_plants.update(consumption_data_exp['Plant_Name'].unique())
    if not consumption_data_act.empty:
        all_plants.update(consumption_data_act['Plant_Name'].unique())
    ...
    if not storage_data_exp.empty and 'Material' in storage_data_exp.columns and not storage_data_act.empty and 'Material' in storage_data_act.columns:
        ...
        for material in sorted(list(all_materials)):
            ...
            # --- MODIFICATION: Pass market data to the storage plotting function ---
            plot_storage_data(material, material_df_exp, material_df_act, market_data_full, opportunity_cost, storage_plot_dir)
    else:
        print("INFO: No storage data found or 'Material' column missing. Skipping storage plots.")


    h2_df_exp = loaded_data.get("hydrogen_prod_file_expected", pd.DataFrame())
    h2_df_act = loaded_data.get("hydrogen_prod_file_actual", pd.DataFrame())
    prod_summary_dir = os.path.join(plot_main_dir, "production_summary")
    os.makedirs(prod_summary_dir, exist_ok=True)
    plot_cumulative_hydrogen_production(h2_df_exp, h2_df_act, market_data_full, opportunity_cost, config, prod_summary_dir)

    elec_dir = os.path.join(plot_main_dir, "electricity_analysis")
    os.makedirs(elec_dir, exist_ok=True)
    # --- MODIFICATION: Pass market data to the electricity overview plotting function ---
    plot_total_electricity_overview(consumption_data_exp, loaded_data.get('accepted_bids_df_full'), consumption_data_act, market_data_full, opportunity_cost, config, elec_dir)
    plot_unused_cleared_energy(loaded_data.get('unused_energy_df'), elec_dir)

    forecast_periods_info = loaded_data.get('forecast_errors')
...
if __name__ == "__main__":
    try:
        ...
        print(f"Error: {e}")
        traceback.print_exc()
        print("Please ensure 'plot_electrolyzer_simulation_results.py' is in the same directory as 'config.yaml'.")
        sys.exit(1)
